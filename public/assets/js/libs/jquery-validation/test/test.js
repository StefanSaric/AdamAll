...
test("valid() plugin method", function() {
	var form = $("#userForm");
	form.validate();
	ok ( !form.valid(), "forms isn't valid yet" );
	var input = $("#username");
	ok ( !input.valid(), "Input isn't valid either" );
	input.val("Hello world");
	ok ( form.valid(), "forms is now valid" );
...
test("errorcontainer, show/hide only on submit", function() {
	...
	$("#lastname").val("abc");
	ok( v.form(), "forms now valid, trigger showErrors but not invalid-form" );
});
